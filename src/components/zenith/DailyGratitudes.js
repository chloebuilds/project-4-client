...
function DailyGratitudes() {
  ...
  React.useEffect(() => {
    ...
    const fillerGratitudes = [...Array(3)].map(() => ({ id: null, dailyGratitude: '' })) // create placeholder/filler objects (in case there are fewer than 3 gratitudes coming back from the back-end) eg.  this morning I was only grateful for chocolate and nothing else. So this will fill the other 2 with the fillerGratitudes objects (this will also mean that the remaining 2 can be filled in)

    const temporaryGratitudes = [ // making sure that we have at least 3 objects that we can use to create the state we need with the .reduce()
    ...
    const syncedGratitudes = temporaryGratitudes.reduce( // transforming the array of back-end gratitude objects/placeholders into a new object with .reduce() so that we can update state in the format that we need for the UI ({ id: ?, draft: ?, final: ? })
      (newState, gratitude, i) => ({ // The callback function returns is going to be the accumulator on the next iteration or that is returned from the whole reduce (when you get to the last element in the array). The object we are returning is going to be the new state
        ...newState, // spreading over the newState (the accumulator)
        [`gratitude${i + 1}`]: { draft: '', final: gratitude.dailyGratitude, id: gratitude.id }, // converting the object from the back-end OR the placeholder into the shape of object that we need for the front-end and to set in state
      }),
  ...
  const handleBlur = async (e) => { // this is where we update the back-end
    ...
    setGratitudes({ ...gratitudes, [e.target.name]: { draft: '', final: e.target.value, id } })
  }
            ...
            <div key={label}>
              ...
              {gratitude.final ? ( // conditionally rendering whether a p tag or an input is displayed, depending on if there is a gratitude.final that we have set in the useEffect
                <>
                  ...
                  <span id={label} onClick={handleEdit} style= {{ marginLeft: 10 }}><FontAwesomeIcon icon={faEdit} /></span>
                </>
              ) : (
                <Styled.Input
