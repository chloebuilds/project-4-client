...
function NewSprintGoals() {
  ...
  const [sprintGoals, setSprintGoals] = React.useState({ // creating state to hold the 3 gratitudes (3 because we decided on 3). State ALWAYS needs to have 3 gratitudes in it because we are mapping over the objects in state to render a <p> tag/input
  ...
  React.useEffect(() => {
    if (!currentSprint) { // checking if current sprint is defined yet
      return
    }
    const fillerSprintGoals = [...Array(3)].map(() => ({ id: null, goalName: '' })) // create placeholder/filler objects (in case there are fewer than 3 gratitudes coming back from the back-end) eg.  this morning I was only grateful for chocolate and nothing else. So this will fill the other 2 with the fillerGratitudes objects (this will also mean that the remaining 2 can be filled in)

    const temporarySprintGoals = [ // making sure that we have at least 3 objects that we can use to create the state we need with the .reduce()
    ...
    const syncedSprintGoals = temporarySprintGoals.reduce( // transforming the array of back-end gratitude objects/placeholders into a new object with .reduce() so that we can update state in the format that we need for the UI ({ id: ?, draft: ?, final: ? })
      (newState, sprintGoal, i) => ({ // The callback function returns is going to be the accumulator on the next iteration or that is returned from the whole reduce (when you get to the last element in the array). The object we are returning is going to be the new state
        ...newState, // spreading over the newState (the accumulator)
        [`sprintGoals${i + 1}`]: { draft: '', final: sprintGoal.goalName, id: sprintGoal.id }, // converting the object from the back-end OR the placeholder into the shape of object that we need for the front-end and to set in state
      }),
  ...
  const handleChange = (e) => {
    setSprintGoals({ // updating the state
      ...sprintGoals,
      [e.target.name]: { draft: e.target.value, final: '', id: sprintGoals[e.target.name].id }, // takes the name on the input and updating that property so that it's draft is the value of whats in the input. The final is a blank string so that we continue to render the input and the id is whatever it already was - either null or the id from the back-end. This is just updating the state.
    })
  }

  const handleBlur = async (e) => { // this is where we update the back-end
    ...
    setSprintGoals({ ...sprintGoals, [e.target.name]: { draft: '', final: e.target.value, id } })
  }

  const handleKeyDown = (event, index) => { // firing when any key is pressed but we only care if its the enter key
    if (event.key !== 'Enter') {
      return
    }
    if (index < 2) {
    ...
    <>
      ...
      {sprintGoals &&
        Object.entries(sprintGoals).map(([label, sprintGoal], i) => { // mapping over our state. Object.entries means that we can convert the object into an array of arrays where the first element of each sub-array is the property's key (eg. gratitude1, gratitude2..) and the second element of the sub-array is the property's value (the object of the gratitude - { draft: '', final: '', id: null }) SEE LINE 127 above for const array.
          return (
            <div key={label}>
              <label style= {{ display: 'inline-block', width: 20 }}>{i + 1}. </label>
              {sprintGoal.final ? ( // conditionally rendering whether a p tag or an input is displayed, depending on if there is a gratitude.final that we have set in the useEffect
                <>
                  <Styled.P ref={inputRefs.current[i]}>{sprintGoal.final}</Styled.P>
